...
export type DotNotationKeyOf<T extends Record<string, unknown>> = {
    [K in keyof Required<T>]: K extends string ? Required<T>[K] extends Record<string, unknown> ? K | `${K}.${DotNotationKeyOf<Required<T>[K]>}` : K : never;
}[keyof T];
export type DotNotationValueOf<T extends Record<string, unknown>, K extends DotNotationKeyOf<T>> = K extends `${infer Head}.${infer Tail}` ? Head extends keyof T ? T[Head] extends Record<string, unknown> ? Tail extends DotNotationKeyOf<T[Head]> ? DotNotationValueOf<T[Head], Tail> : never : Required<T>[Head] extends Record<string, unknown> ? Tail extends DotNotationKeyOf<Required<T>[Head]> ? DotNotationValueOf<Required<T>[Head], Tail> | undefined : never : never : never : K extends keyof T ? T[K] : never;
type ImmutablePrimitives = Date | RegExp | URL | Error;
export type PartialObjectDeep<T> = T extends ImmutablePrimitives ? T : T extends Map<infer K, infer V> ? Map<PartialObjectDeep<K>, PartialObjectDeep<V>> : T extends Set<infer U> ? Set<PartialObjectDeep<U>> : T extends Array<infer U> ? Array<PartialObjectDeep<U>> : T extends (...args: unknown[]) => unknown ? T : T extends Record<string, unknown> ? {
